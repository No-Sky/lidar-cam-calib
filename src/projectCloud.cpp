...
string input_bag_path, input_photo_path, output_path, intrinsic_path,
    extrinsic_path;
...
void loadPointcloudFromROSBag(const string &bag_path) {
  ...
  vector<string> types;
  types.push_back(string("/rslidar_mid_points")); // message title
  rosbag::View view(bag, rosbag::TypeQuery(types));
  ROS_INFO("rslidar_mid_points");
  ROS_INFO(std::to_string(view.size()).c_str());
  for (const rosbag::MessageInstance &m : view) {
    sensor_msgs::PointCloud2 lidar_data =
        *(m.instantiate<sensor_msgs::PointCloud2>()); // message type
    ROS_INFO(std::to_string(lidar_data.width).c_str());
    lidar_datas.push_back(lidar_data);
...
void getColor(int &result_r, int &result_g, int &result_b, float cur_depth) {
  ...
  if (cur_depth < min_depth) {
    ...
    result_g = 0xff;
    result_b =
        (0xff - int((cur_depth - min_depth - scale) / scale * 255)) & 0xff;
    ...
    result_r = 0xff;
    result_g =
        (0xff - int((cur_depth - min_depth - scale * 4) / scale * 255)) & 0xff;
    result_b = 0;
...
int main(int argc, char **argv) {
  ...
  // pcl::io::loadPCDFile<pcl::PointXYZ>(input_bag_path, cloud);
  pcl::io::loadPCDFile<pcl::PointXYZ>(input_bag_path, *cloud);

//   pcl::PointCloud<pcl::PointXYZI>::Ptr filtered_body{new pcl::PointCloud<
//       pcl::PointXYZI>}; //指针还是对象，有时候只能指针，有时候都行。报错就换。
//   pcl::CropBox<pcl::PointXYZI> box_filter; //滤波器对象
//   // Min和Max是指立方体的两个对角点。每个点由一个四维向量表示，通常最后一个是1.
//   box_filter.setMin(Eigen::Vector4f(-1.575476, 0.652452, -0.049252, 1.0));
//   box_filter.setMax(Eigen::Vector4f(-1.465442, -0.535379, 0.053488, 1.0));
//   //是保留立方体内的点而去除其他点，还是反之。false是将盒子内的点去除，默认为falseuu
//   box_filter.setNegative(false);
//   box_filter.setInputCloud(cloud);   //输入源
//   box_filter.filter(*filtered_body); //滤它！
//   ROS_INFO(to_string(filtered_body->size()).c_str());
  ...
  //     pcl::fromROSMsg(lidar_datas[i], cloud);
  ...
  for (unsigned int j = 0; j < cloud->size(); ++j) {
    ...
    getTheoreticalUV(theoryUV, intrinsic, extrinsic, x, y, z);
    if (theoryUV[0] <= 0 || theoryUV[0] >= 1920 || theoryUV[1] <= 0 ||
        theoryUV[1] >= 1080) {
      continue;
    }
    int u = floor(theoryUV[0] + 0.5);
  ...
  cv::remap(src_img, src_img, map1, map2,
            cv::INTER_LINEAR); // correct the distortion
