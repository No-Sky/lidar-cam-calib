...
void on_mouse(int event, int x, int y, int flags, void *param) {
  ...
  if (event == cv::EVENT_LBUTTONDOWN) {
    ...
    // + std::to_string(static_cast<int>(img->at<uchar>(cv::Point(x, y))));
    // cv::putText(*img, text, cv::Point(0, img->rows),
...
int main(int argc, char **argv) {
  ...
  //     cv::getOptimalNewCameraMatrix(cameraMatrix, distCoeffs, imageSize, 1);
  // cv::Mat new_img;
  // cv::undistort(src_img, new_img, new_camera_matric, distCoeffs);

  cout << "Please note the four corners, and then tap a random key to give the "
          "coordinate"
       << endl;
  // cv::namedWindow("source", CV_WINDOW_KEEPRATIO);
  cv::namedWindow("source");
  cv::setMouseCallback("source", on_mouse, reinterpret_cast<void *>(&src_img));
  ...
  for (uint t = 0; t < corners.size(); ++t) {
    cv::circle(result_img, corners[t], 3,
               cv::Scalar(random_number_generator.uniform(0, 255),
                          random_number_generator.uniform(0, 255),
                          random_number_generator.uniform(0, 255)),
               1, 8, 0);
